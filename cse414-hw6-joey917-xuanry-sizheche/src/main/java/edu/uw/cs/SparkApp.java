 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> r = rdd.filter(line -> line.getString(ORIGIN_CITY_NAME).equals("Seattle, WA")).
                map(t -> RowFactory.create(t.getString(DEST_CITY_NAME))).distinct();
        // TODO your code here
        return r;
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
//        JavaRDD<Row> rdd = d.javaRDD();
//        JavaPairRDD<Tuple2<String,String>,Integer> r = rdd.filter(line -> line.getInt(CANCELLED)!=-1).
//                mapToPair(line -> new Tuple2<Tuple2<String,String>,Integer>(
//                        new Tuple2<String,String>(line.getString(ORIGIN_CITY_NAME),line.getString(MONTH)),1));
        Dataset<Row> result = d.filter(d.col("cancelled").$eq$bang$eq(1)).
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        // TODO your code here
//        JavaRDD<Row> rdd = dSparkSession.javaRDD();
        Dataset<Row> result = d.filter(d.col("depdelayminutes").isNotNull()).
                filter(d.col("cancelled").$eq$bang$eq(1)).
